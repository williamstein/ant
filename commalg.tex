...
\begin{enumerate}
	...
	step is done the upper left entry of the matrix is nonzero, and all other
...
$f_i$ has leading coefficient $a_i$, the difference $g- x^{e-d} (\sum_i r_i f_i)$ 
has degree less than the degree~$e$ of~$g$.  By induction 
$g-x^{e-d} (\sum_i r_i f_i)$ is an $R[x]$ linear combination of 
$f_1,\ldots, f_n, h_1,\ldots, h_m$, so $g$ is also an $R[x]$ linear combination of
...
\begin{remark}
	...
	product of algebraic integers is an algebraic integer. In particular, 
	if $\alpha \in \O_L$ and $K=\Q$, then the norm and trace of $\alpha$ is an integer. 
\end{remark}
