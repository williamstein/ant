...
\begin{example}
The matrix $\mtwo{-1}{2}{-3}{4}$ has Smith normal form of
$\mtwo{1}{0}{0}{2}$,
and the matrix 
...
\begin{enumerate}
	\item Use Theorem~\ref{thm:struc} to classify all abelian groups of order less than $12$. How many do you think there are? How many are there?
	
	\item Recall Smith normal form defined in Proposition~\ref{prop:smith}. With only minor modifications, the proposition and proof will work over any principle ideal domain. Find and apply these modifications then find the Smith normal form of the matrix $\begin{pmatrix} 1 & 2 & 3 \\ 0 & 1+i & 2 \\ 0 & 1 & 5 \end{pmatrix}$. Hint: you can use \sage{} to verify your answer.
	
	\item There is another way to show every principle ideal domain (for example $\ZZ$) is noetherian (in contrast to Section~\ref{sec:Znoeth}). Let $R$ be a PID and $(a)$ an arbitrary ideal. Use the facts that $(b)\supseteq(a)$ if and only if $b \mid a$ and $R$ is a UFD to show that ascending chain of ideals starting with $(a)$ must stabilize.
