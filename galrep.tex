...
E.plot(pointsize=50, gridlines=True)
\end{sagecell}
...
if it is vertical, i.e. is of the form $x=a$ for some fixed $a\in K$.
...
on a common line (given by $x = 3$), so $(3,4)=-(3,-4)$.  See the illustration below:
\begin{sagecode}
\begin{sagecell}
E = EllipticCurve([-5,4])
E(0,2) + E(1,0)
...
G += points([(0,2), (1,0), (3,4), (3,-4)],
    pointsize=50, color='red')
G += line([(-1,4), (4,-6)], color='black')
G += line([(3,-5),(3,5)], color='black')
G
\end{sagecell}
\begin{sageout}[escapechar=!] %skip
!\includegraphics[width=0.9\textwidth]{graphics/grouplaw}
\end{sageout}
\end{sagecode} %link
...
%TODO Hartshorne's abstract nonsingular curve is described as
%TODO all the DVRs in a given field. It looks like this is that.
%TODO in general are all DVRs of some field the localization of
%TODO some dedekind domain in the field? Where does this ring
%TODO fit in algebraic geometry, as in, is there some "ring of
...
\end{sagecode}
\noindent
Note that this matches with what we expected, i.e. we found
