...
\begin{remark}
	The fact that $E[n] \approx (\Z/n\Z)^2$ holds over fields of
	characteristic $p>0$ as well, with special exceptions
	when $p|n$. However, the methods we used above will not apply
	to the case of positive characteristic. Another method is to
	compute the degree of the multiplication by $n$ map and show
	it is separable. For more a detailed proof see
