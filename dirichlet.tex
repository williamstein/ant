...
the same as those described in Lemma~\ref{lem:volok}. So in particular
$\sigma_1,\dots,\sigma_r$ represent all real embeddings
$K\to\R$ and $\sigma{r+1},\dots,\sigma_{r+s}$ represent
half of the complex embeddings $K\to\C$, one representative
...
\begin{proof}
  ...
  order~$n$.) %TODO change to excercise?
\end{proof}
...
Recall form Example~\ref{example:Qsqrt5ringofints} that the
...
We can also list out the coefficients for these powers as follows.
...
\end{sagecell}
\begin{sageout} %TODO check these results aren't being cached?
