...
\begin{remark}
	...
	cohomology groups on the etale site over $\Spec K$.
	One can show that \v{C}ech cohomology
	(in a fairly general setting) agrees with
	the derived functor groups of $A\mapsto A^G$,
	see \cite[Ch.~10]{milne:etale}. Therefore
	$\H^n(K,A)$ do indeed define a cohomology theory.
\end{remark}
...
\begin{proof}
	...
	Let $G = \Gal(L/K)$ and $f$ be a $1$-cocycle so that $f:G \to L^*$ such
	that $f(\sigma\tau) = f(\sigma)\cdot\sigma(f(\tau))$. Here
